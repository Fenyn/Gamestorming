...
- **Action bar â€” the anchor surface.** It passes all four tests outright: every element on it changes within the turn and gets read under pressure. It inherits the size budget the killed and demoted panels gave up. Actor identity, HP vitals, three action pips, action buttons with hotkey captions, End Turn.
...
- **Pick state â€” accent border plus caption.** `RosterCardSelected`'s 2 px accent border and "STARTING CHARACTER". Exactly one card is picked, so there is no slot number left to disambiguate and no strip needed.
...
| Action | Keys | Meaning |
| --- | --- | --- |
| `combat_action_1..4` | 1â€“4, Kp 1â€“4 | Action bar buttons |
...
The optional Dice rolls toggle shows a short reveal of the latest resolved d20 without changing gameplay RNG. It defaults off and persists for the session. The popup shows only rolls retained in the engine log, not reconstructed spell-save dice. AI action cues pause for 0.35 seconds; movement tiles and individual area-damage targets add no extra pause.
