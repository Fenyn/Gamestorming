...
func _ready() -> void:
	for i: int in CELL_COUNT:
		var cell: PowerCell = _viewport.get_node("PowerCell" + str(i)) as PowerCell
		if cell:
			cell.cell_index = i
		...
		label.add_theme_color_override("font_color", Color(0.3, 0.9, 0.95))
...
func _process(_delta: float) -> void:
	...
	var scale: Vector2 = container_size / vp_size
	...
	for i: int in _cells.size():
		...
		var screen_pos: Vector2 = _camera.unproject_position(top_pos) * scale
...
func _get_drag_data(at_position: Vector2) -> Variant:
	...
	preview.add_theme_color_override("font_color", ThemeBuilder.ACCENT_GLOW)
