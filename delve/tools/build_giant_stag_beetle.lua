...
local swatches={m='995e45',s='633d32',d='34312e',l='c7986c',o='1e2222',c='666857',t='484b41',h='969981',e='ffc96c'}
local P={}; local palette=Palette(10); palette:setColor(0,Color{r=0,g=0,b=0,a=0})
for i,k in ipairs({'m','s','d','l','o','c','t','h','e'}) do
...
local function shadeLine(im,a,b,r1,r2,k)
 local mask=Image(W,H,ColorMode.RGB); line(mask,a,b,r1,r2,k)
 for y=0,H-1 do for x=0,W-1 do
  if app.pixelColor.rgbaA(im:getPixel(x,y))>0 and app.pixelColor.rgbaA(mask:getPixel(x,y))>0 then im:drawPixel(x,y,mask:getPixel(x,y)) end
 end end
end
...
local function pose(dx,dy,jaw)
 ...
 for n,chain in ipairs(legs) do
  ...
  line(im,a,b,2,1.7,near and 'c' or 't'); line(im,b,c,1.7,.6,near and 'c' or 't')
  line(im,{c[1]-1,c[2]}, {c[1]+2,c[2]},.6,.4,'o')
  if near then
   -- A continuous lower femur plane tapers into the knee; medium value avoids zipper contrast.
   shadeLine(im,{a[1],a[2]+1},{b[1],b[2]+.6},1,.7,'t')
   shadeLine(im,{b[1]+.6,b[2]+1},{b[1]+(c[1]-b[1])*.8+.4,b[2]+(c[2]-b[2])*.8},.9,.6,'t')
 ...
 poly(4,{{11,33},{16,36},{24,38},{33,38},{40,35},{44,30},{45,25},{48,28},{48,36},{43,43},{33,46},{21,44},{13,39}},'s')
 ...
 poly(5,{{43,33},{48,36},{53,35},{55,31},{57,34},{55,42},{49,45},{44,41}},'t')
 poly(5,{{44,40},{49,42},{54,40},{55,42},{49,45}},'d')
 ...
 poly(6,{{53,37},{58,39},{63,38},{66,35},{66,41},{61,46},{55,44},{52,39}},'t')
 poly(6,{{55,42},{61,44},{65,41},{61,46},{55,44}},'d')
 -- Contact edge belongs to the front head, separating it from pronotum behind.
 poly(6,{{55,30},{56,31},{54,35},{54,39},{56,42},{55,44},{52,39},{53,34}},'d')
