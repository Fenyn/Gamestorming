class_name CrewWanderState3D
extends BaseState

const STUCK_THRESHOLD: float = 2.0
const PHASE_DURATION: float = 1.5
const MIN_MOVE_DISTANCE: float = 0.05
...
func enter(_msg: Dictionary = {}) -> void:
	...
	var offset: Vector3 = Vector3(
		randf_range(-CrewMember3D.WANDER_RADIUS, CrewMember3D.WANDER_RADIUS),
		0.0,
		randf_range(-CrewMember3D.WANDER_RADIUS, CrewMember3D.WANDER_RADIUS)
	)
...
func physics_update(delta: float) -> void:
	...
	if _is_phasing:
		...
		crew.velocity = dir.normalized() * CrewMember3D.SPEED
		crew.move_and_slide()
...
func _begin_phase(crew: CrewMember3D) -> void:
	_is_phasing = true
	_phase_timer = PHASE_DURATION
	crew.collision_mask = 0


func _end_phase(crew: CrewMember3D) -> void:
	_is_phasing = false
	_stuck_timer = 0.0
	crew.collision_mask = 1
