...
local function shade(x,y,i,k)
  ...
  elseif i==1 then
    ...
    if (y>=38 and x<=20) or (y>=43 and x<=19) then return 'd' end
    return 's'
  elseif i==2 then
    if below or (y>=35 and x==30) then return 'd' end
    if y>=36 then return 'd' end
    return 's'
  elseif i==3 then
  ...
  elseif i==9 then
    if inside(x,y,{{14,20},{15,22},{11,26},{7,27},{8,25},{11,24}}) then return 'd' end
    if inside(x,y,{{13,19},{15,20},{12,24},{8,25},{9,23}}) then return 's' end
    ...
    if y<=21 and x>=16 then return 'd' end
    if inside(x,y,{{16,24},{20,25},{22,28},{18,28},{16,26}}) then return 's' end
...
comparison:drawImage(Image{fromFile=out..'/revisions/flat-v3/goblin-base.png'},Point(0,0))
