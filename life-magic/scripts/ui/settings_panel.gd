...
func _build_ui() -> void:
	...
	_age_spin = _add_spin_row("Age", 10, 100, int(GameState.settings.get("age", 30.0)))
	...
	_cap_spin = _add_spin_row("Cap %", 70, 95, int(GameState.settings.get("hr_cap_pct", 0.85) * 100.0))
	_cap_spin.suffix = "%"
	...
	source_desc.text = "Demo: simulated workout cycle. Manual: scroll wheel to set BPM. Device: connect a heart rate monitor via WebSocket."
...
func _build_how_it_works() -> void:
	...
	var lines := [
		"Your heartbeat fuels everything. A higher heart rate makes the game run faster.",
		"",
		"Tick — A production cycle. Every few seconds, all your spells fire at once. The tick timer is shown as a progress bar below your mana. Faster heart rate = shorter ticks.",
		"",
		"Speed — The multiplier from your heart rate. At rest it's 1.0x. Exercise to push it up to 3.0x, making ticks happen three times faster.",
		"",
		"Spells form a chain. Heartmotes produce Mana directly. Higher spells (Pulse Glyphs, Familiars, etc.) produce the spell below them each tick, which in turn produce more of what's below — a cascade of creation.",
		"",
		"Bought vs Bonus — The number you purchased plus bonus units generated by higher-tier spells. Both produce for you every tick.",
		"",
