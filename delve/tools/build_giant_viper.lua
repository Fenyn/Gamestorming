  ...
  for i,original in ipairs(originals) do
   ...
   for y=0,H-1 do for x=0,W-1 do
    ...
    if sx>=0 and sx<W and sy>=0 and sy<H then
     ...
     -- Opening the mouth exposes a shadowed lip, not a continuous tooth row.
     if i==5 and ps[3]>0 and sy>=27 and p==P.c then p=P.s end
     im:drawPixel(x,y,p)
    end
   end end
   ...
   if i==6 and ps[3]>0 then
    ...
    for x=62,72 do local depth=math.floor(ps[3]*(x-62)/10+.5)
     ...
     -- Light belongs to the chin tip only; the jaw turns diagonally from cheek.
     if x>=71 then im:drawPixel(x+shift,30+depth-lift,P.c) end
    end
    im:drawPixel(69+shift,29-lift,P.c)
    im:drawPixel(69+shift,30-lift,P.c)
    if ps[3]>=3 then im:drawPixel(69+shift,31-lift,P.c) end
   end
...
saveClip('attack',{.8,1,.8,1,1.2,1},10,{{-1,0,0},{-3,0,5},{8,0,4},{5,0,4},{1,0,2},{0,0,0}})
